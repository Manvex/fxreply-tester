...
(() => {
  ...
  function openSession() {
    ...
    selected = window.App.currentSymbolInfo;
    fmtSel(selected);
    closeSearch();
    $('#ss-progress').classList.add('hidden');
    // sensible default: ~3 months back
    if (!$('#ss-start').value) {
      const d = new Date(Date.now() - 90 * 86400000);
      $('#ss-start').value = d.toISOString().slice(0, 10);
    }
    const s = window.App.manualSettings;
    $('#ss-balance').value = s.balance;
    $('#ss-leverage').value = String(s.leverage);
    $('#ss-spread').value = s.spread;
    $('#ss-commission').value = s.commission;
    $('#ss-prop').value = s.propMode || 'none';
  ...
  function updateNote() {
    ...
    const [, span] = WINDOW[tf] || WINDOW['1h'];
    const days = Math.round(span / 86400);
    const human = days >= 365 ? (days / 365).toFixed(days % 365 ? 1 : 0) + ' year' + (days >= 730 ? 's' : '') : days + ' days';
    $('#ss-note-text').innerHTML =
      `On <b>${tf.toUpperCase()}</b> the session loads about <b>${human}</b> of real data from your start date forward (plus warm-up history before it). You can jump to a later date at any time without reloading.`;
  }
  ...
  async function startSession() {
    ...
    if (!dateStr) { window.App.toast('Pick a start date first', 'warn'); return; }
    const tf = $('#ss-tf').value;
    const parts = dateStr.split('-').map(Number);
    const startSec = Date.UTC(parts[0], parts[1] - 1, parts[2]) / 1000;
    const now = Math.floor(Date.now() / 1000);
    if (startSec >= now - 86400) { window.App.toast('The start date must be in the past', 'warn'); return; }
    ...
    try {
      ...
      if (candles.length < 30) {
        window.App.toast(`Not enough ${info.sym} data around that date — try a later start or a higher timeframe`, 'err');
        setProgress(false);
        btn.disabled = false;
        return;
      }
      ...
      if (ok) {
        window.App.toast(`Session started — ${info.sym} ${tf.toUpperCase()} from ${dateStr}. Use → to step, Space to play.`, 'ok');
      }
    } catch (e) {
      console.error(e);
      window.App.toast('Could not load data: ' + (e.message || e), 'err');
    } finally {
      setProgress(false);
      btn.disabled = false;
    }
